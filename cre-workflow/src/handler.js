...
var TENDERLY_API_URL = process.env.TENDERLY_API_URL || `https://api.tenderly.co/api/v1/account/${process.env.TENDERLY_ACCOUNT_SLUG || "veridex-protocol"}/project/${process.env.TENDERLY_PROJECT_SLUG || "project"}/simulate`;
var TENDERLY_ACCESS_KEY = process.env.TENDERLY_ACCESS_KEY || "";
var httpTrigger = new HTTPCapability().trigger({});
var evaluateThreat = cre.handler(httpTrigger, async (runtime2, triggerOutput) => {
  console.log("-----------------------------------------");
  ...
  const http = new ClientCapability3;
  let simulationSuccess = false;
  let simulationGasUsed = 0;
  if (TENDERLY_ACCESS_KEY) {
    try {
      const simulateFn = runtime2.runInNodeMode((nodeRuntime) => {
